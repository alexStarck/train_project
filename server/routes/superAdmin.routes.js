...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const candidate = await SuperAdmin.findOne({login: 'master'})
            if (!candidate) {
                const pass = await bcrypt.hash(process.env.DefaultPass, 12)
                await SuperAdmin.create({login: 'master', password: pass})
            }
