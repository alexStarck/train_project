    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                ...
                <Button icon="pi pi-trash" className="p-button-rounded p-button-warning" onClick={() => confirmDeleteObject(rowData)} />
            </React.Fragment>
        );
    }
            ...
            <div>
                <div className='p-col-6'>
                    <React.Fragment >
                        ...
                        <Button label="Удалить" icon="pi pi-trash" className="p-button-danger p-mr-2" onClick={confirmDeleteSelected}  disabled={!selectedObjects || !selectedObjects.length} />
                    </React.Fragment>
                </div>
        ...
        <div className="p-grid crud-demo">
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={objectsList} selection={selectedObjects} onSelectionChange={(e) =>{setSelectedObjects(e.value)} }
                    ...
                    <Column field="detail" header="Детали" headerStyle={{ width: '500px' }} body={DetailBodyTemplate} />
                    <Column body={actionBodyTemplate}/>
                </DataTable>
            </div>

            <Dialog visible={objectEditDialog} style={{ width: '650px' }} header="Редактирование поезда" modal className="p-fluid" footer={objectEditDialogFooter} onHide={hideDialog} position='top'>

                <div className="p-field">
                    <label htmlFor="objectName">Номер</label>
                    <InputText id="objectName" value={object.objectName} onChange={(e) => onInputChange(e, 'objectName')} required autoFocus  />
                    {submitted && !object.objectName && <small className="p-error">objectName is required.</small>}
                </div>

                <div className="p-field">
                    <label htmlFor="type">Тип</label>
                    <InputText id="type" value={object.type} onChange={(e) => onInputChange(e, 'type')} required autoFocus  />
                ...
                <div className="p-field" style={{height:height}}>
                    ...
                    <Dropdown value={valueComposition} options={valuesCom} onChange={onValueCompositionChange} optionLabel="name" placeholder="Select a value of composition" />
                </div>
                {composition!==null && (composition.map((item,index)=>{
                        return(
                            <div className="p-mb-2 p-d-flex " key={index}>
                                ...
                                <Dropdown className="p-d-inline-flex"  value={item.class} options={classList} onChange={(e) => onCompositionChange(e, item.number)}  placeholder="тип вагона"  minLength={2} maxLength={2}  editable required  />
                ...
                <div className="p-field">
                    ...
                    <InputText id="detail" value={object.detail} onChange={(e) => onInputChange(e, 'detail')} required autoFocus  />
                    {submitted && !object.detail && <small className="p-error">detail is required.</small>}
                </div>
            </Dialog>

            <Dialog visible={objectDialog} style={{ width: '650px' }} header="Создание поезда" modal className="p-fluid" footer={objectDialogFooter} onHide={hideDialog}position='top'>
                <div className="p-field">
                    <label htmlFor="objectName">Номер</label>
                    <InputText id="objectName" value={object.objectName} onChange={(e) => onInputChange(e, 'objectName')} required autoFocus  />
                    {submitted && !object.objectName && <small className="p-error">objectName is required.</small>}
                ...
                {composition!==null && (composition.map((item,index)=>{
                        return(
                            <div className="p-mb-2 p-d-flex " key={index}>
                                <InputText className="p-mr-2 p-d-inline-flex" value={item.number}   disabled   />
                                <InputText className="p-d-inline-flex" value={item.class} keyfilter={/^[0-9а-яА-Я]+$/} onChange={(e) => onCompositionChange(e, item.number)} placeholder="тип вагона" minLength={2} maxLength={2} required   />
                            </div>
                        )
                    })
                )}
            ...
            <Dialog visible={deleteObjectDialog} style={{ width: '450px' }} header="Подтверждение" modal footer={deleteObjectDialogFooter} onHide={hideDeleteObjectDialog} position='top'>
            ...
            <Dialog visible={deleteObjectsDialog} style={{ width: '450px' }} header="Подтверждение" modal footer={deleteObjectsDialogFooter} onHide={hideDeleteObjectsDialog} position='top'>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle p-mr-3" style={{ fontSize: '2rem'}} />
